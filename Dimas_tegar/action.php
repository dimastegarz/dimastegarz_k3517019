<?php
	...
	if (isset($_POST['submit1'])){
		...
		$_SESSION['filenameupload'] = $uploadfile;
	 	setcookie("username", $_POST['username'], time()+3600*24*7,"/Dimas_tegar");
	 	header("Location: action.php");
	}else if(isset($_POST['submit2'])){
		$userInfo = $_COOKIE['username'];
		setcookie("usename", $userInfo, time()+3600*24*7,"/Dimas_tegar");
		//header("Location: action.php");
	}
...
<body>
	...
	<?php
		...
		if (isset($status)){
			...
			if ($status == true){
				echo "<h3>Jawaban Betul bro</h3>";
			} else {
				// jika status = false, maka munculkan 'jawaban salah'
				echo "<h3>Jawaban Salah bro</h3>";
			}
		}
	?>
	...
	<?php
		...
		if ($_SESSION['lives'] == 0){
			...
			$query = "INSERT INTO skor (username, score, playtime, foto) VALUES ('".$_COOKIE['username']."', ".$_SESSION['score'].", '".date('Y-m-d H:i:s')."','".$_SESSION['filenameupload']."')";
			$hasil = mysqli_query($db, $query) or die(mysqli_error(1));
